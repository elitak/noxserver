	...
	dxBody * const *body, dRealPtr invI)
{
	...
	for (i=0; i<m; i++) {
		...
		for (j=0; j<3; j++) iMJ_ptr[j] = k*J_ptr[j]; // Philip
		dMULTIPLY0_331 (iMJ_ptr + 3, invI + 12*b1, J_ptr + 3);
		if (b2 >= 0) {
			k = body[b2]->invMass;
			for (j=0; j<3; j++) iMJ_ptr[j+6] = k*J_ptr[j+6]; // Philip
		     ...
		     dxJoint * const *_joint, int nj, dReal stepsize)
{
	...
	for (i=0; i<nb; i++) {
		if ((body[i]->flags & dxBodyNoGravity)==0) {
			...
			//body[i]->facc[2] += body[i]->mass.mass * world->gravity[2]; // Philip
            body[i]->facc[2] = 0;
		}
	}
	...
	if (m > 0) {
		...
		Jinfo.rowskip = 12;
		Jinfo.fps = stepsize1;
		//Jinfo.erp = world->global_erp; // Philip
		for (i=0; i<nj; i++) {
		...
		for (i=0; i<nb; i++) {
			...
			for (j=0; j<3; j++) tmp1[i*6+j] = body[i]->facc[j] * body_invMass + body[i]->lvel[j] * stepsize1; // Philip
			dMULTIPLY0_331 (tmp1 + i*6 + 3,invI + i*12,body[i]->tacc);
			for (j=0; j<3; j++) tmp1[i*6+3+j] += body[i]->avel[j] * stepsize1; // Philip
		}
		...
		for (i=0; i<nb; i++) {
			for (j=0; j<3; j++) body[i]->lvel[j] += stepsize * cforce[i*6+j]; // Philip
			for (j=0; j<3; j++) body[i]->avel[j] += stepsize * cforce[i*6+3+j]; // Philip
		}
		...
		for (j=0; j<nj; j++) {
			if (joint[j]->feedback) {
				...
				for (i=0; i<nb; i++) {
					...
					cforce [i*6+0] *= k;
					cforce [i*6+1] *= k;
					//cforce [i*6+2] *= k; // Philip
                    cforce [i*6+2] *= 0;
					dVector3 tmp;
					dMULTIPLY0_331 (tmp, I + 12*i, cforce + i*6 + 3);
					cforce [i*6+3] = tmp[0];
					cforce [i*6+4] = tmp[1];
					//cforce [i*6+5] = tmp[2]; // Philip
                    cforce [i*6+5] = 0; // Philip
				}
	...
	for (i=0; i<nb; i++) {
		dReal body_invMass = body[i]->invMass;
		for (j=0; j<3; j++) body[i]->lvel[j] += stepsize * body_invMass * body[i]->facc[j]; // Philip
		for (j=0; j<3; j++) body[i]->tacc[j] *= stepsize;	 // Philip
	...
	for (i=0; i<nb; i++) {
		dSetZero (body[i]->facc,3); // Philip
		dSetZero (body[i]->tacc,3); // Philip
	}
