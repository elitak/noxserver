...
namespace Flatland
{
    ...
    class Object
    {
      public:
		...
		void SetUserPointer(void* ptr) { usrPointer = ptr; }
		void* GetUserPointer() { return usrPointer; }
      public:
    ...
    class World
    {
      public:
        ...
        Body BodyCreate();
        template<class S> void GenerateContacts(const S &space, const S &players);
	...
	template<class Container>
	void World::GenerateContacts(const Container &space, const Container &players)
    {
        dJointGroupEmpty(contactGroup);
        contactCount = 0;
        typename Container::const_iterator o1;
        for (o1 = players.begin(); o1 != players.end(); ++o1)
        {
            Object *object1 = (*o1)->GetFlatlandObject();
            if (!object1)
                continue;

            typename Container::const_iterator o2;
            for (o2 = space.begin(); o2 != space.end(); ++o2)
            {
                Object *object2 = (*o2)->GetFlatlandObject();
                if (!object2 || object2 == object1)
                    continue;
